...
def main():
    ...
    profile_name = os.path.splitext(
        os.path.basename(os.path.abspath(args_app.profile))
    )[0]
    ...
    artifact_version = (
        find_artifact_version(config) if "revision" in args_app else args_app.revision
    )
    ...
    archive_root = make_absolute_path(
        config["execution"]["archive-dir"], args_app.profile
    )
    if os.path.exists(
