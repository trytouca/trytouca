...
scp ops.tar.gz touca@your-machine:~/
tar -zxf ../ops.tar.gz
rm ../ops.tar.gz
```
...
`ops/docker-compose.prod.yaml` file. Do not wrap the values in single or double
quotations.
...
Now run `ops/deploy.sh` to deploy Touca via Docker Compose.

```bash
~/touca/ops/deploy.sh -r <TOUCA_AWS_REPO> -u AWS
```
...
```bash
docker-compose -f ~/touca/ops/docker-compose.prod.yml --project-directory ~/touca logs --follow
```
