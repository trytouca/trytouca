...
We use `docker-compose` to pull Touca docker images and run them. Once the
install is over, navigate to Touca server and complete the install wizard.

## Uninstall Touca Server
...
Both commands stop the running containers and remove all server data from the
filesystem. Run `rm -r $HOME/.touca/results` if you like to remove any locally
stored test results.
