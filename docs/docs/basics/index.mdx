...
your API Key and URL to complete this hands-on tutorial.
...
sample data for version `v1.0` of our `hard helmets` example and makes them
...
have captured the output of our software, including a JSON file listing the
...
Now if we make changes to our software, we can run the same command to compare
subsequent versions against our baseline.
...
Unlike snapshot testing, Touca stores the test results on a remote server. The
server automatically compares the results for newly submitted versions against
our baseline version and reports the differences in near real-time. In our
example, we can use the Touca server to see how `v1.1` of our `hard helmets`
