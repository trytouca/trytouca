...
The quickest way to configure the CLI is to call `touca login` to log into your
Touca account from the command line. This will direct you to a browser window
where you can log into or create an account, so an API key is automatically set
for you that the CLI will use.

By default, `https://api.touca.io` is used as the API URL if it has not been
configured otherwise. You may also provide a URL to a self-hosted instance using
the `--api-url` option.
