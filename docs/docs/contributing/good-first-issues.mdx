...
### 5. Create a Unified Docker image for Touca Server

Touca server is currently split into two containers:
...
docker image that runs both `nginx` and `node` processes.
...
### 6. Add Terraform configuration for AWS
...
### 8. Create package recipe for ConanCenter
...
### 9. Improve CMake logic for packaging
