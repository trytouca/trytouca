...
export async function generateStaticParams() {
  return await generateArticlesStaticParams('changelog');
}

export default async function Page({ params }: { params: { slug: string } }) {
  const { main_article, archived_articles } = await fetchArticle(
    'changelog',
    params.slug
  );
  return (
    <main>
      ...
      {archived_articles.length !== 0 && (
        <section className="bg-dark-blue-900">
          <BlogPostArchive articles={archived_articles} parent="changelog" />
        </section>
      )}
      <Script
